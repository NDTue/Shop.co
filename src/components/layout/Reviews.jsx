...
function Reviews(props) {
  ...
  // Bắt đầu với index 2 (card thứ 3) để hiển thị card 3, 4, 5 lúc khởi tạo
  const [currentIndex, setCurrentIndex] = useState(2)

  // State để kiểm soát animation
  // Khi true, animation sẽ được áp dụng  false khi muốn reset vị trí mà không có animation
  const [isAnimating, setIsAnimating] = useState(false)

  // Tham chiếu đến phần tử DOM carousel để thao tác trực tiếp
  const carouselRef = useRef(null)


  // Hàm xử lý khi người dùng nhấn nút Previous (trước đó)
  const handlePrev = () => {
    // Đặt trạng thái đang animation để kích hoạt hiệu ứng CSS
    setIsAnimating(true)

    // Giảm index hiện tại, nếu ở vị trí đầu tiên thì quay lại vị trí cuối
  ...
  const handleNext = () => {
    ...
    // Tăng index hiện tại, nếu ở vị trí cuối thì quay lại vị trí đầu
    setCurrentIndex((prevIndex) =>
      prevIndex === reviewData.length - 1 ? 0 : prevIndex + 1
    )
  }

  // Tính toán các cards nên hiển thị trên màn hình mobile (chỉ một card)
  const getMobileVisibleCard = () => {
    // Trên mobile, chỉ hiển thị 1 card tại vị trí currentIndex
    return reviewData[currentIndex]
  }

  // Tính toán các cards nên hiển thị trên màn hình desktop (3 cards)
  const getDesktopVisibleCards = () => {
    // Trên desktop, hiển thị 3 cards liên tiếp từ vị trí currentIndex
    return [
      // Lấy card hiện tại
      reviewData[currentIndex],
      // Lấy card tiếp theo, nếu vượt quá độ dài mảng thì quay lại đầu mảng
      reviewData[(currentIndex + 1) % reviewData.length],
      // Lấy card tiếp theo nữa
      reviewData[(currentIndex + 2) % reviewData.length]
    ]
  }

  // Effect để reset trạng thái animation sau khi animation hoàn thành
  useEffect(() => {
    if (isAnimating) {
      // Đặt timeout để tương ứng với thời gian của animation (500ms)
      const timer = setTimeout(() => {
        setIsAnimating(false)
      }, 500)

      // Dọn dẹp timeout khi component unmount hoặc re-render
      return () => clearTimeout(timer)
    }
  }, [isAnimating])

  // Hàm hiển thị một card đánh giá (thay thế cho component ReviewCard)
  const renderReviewCard = (review, isLastVisible) => {
    return (
      // Thêm border-r-0 cho card cuối cùng để sửa lỗi viền
      <div className='h-full border-2 border-[#e6e6e6] rounded-3xl px-6 py-6 transition-shadow'>
        ...
        {/* Thông tin người dùng với badge xác minh */}
  ...
  return (
    <section className="relative max-w-96 md:max-w-6xl mx-auto mt-10 pb-16 px-4">
      ...
      {/* Container chứa carousel với overflow hidden để ẩn thanh cuộn */}
      <div className="relative overflow-hidden">
        {/* Hiển thị mobile - một card duy nhất */}
        <div className="md:hidden">
          <div className={`transition-transform duration-500 ease-in-out ${isAnimating ? 'transform translate-x-full' : ''}`}>
            {renderReviewCard(getMobileVisibleCard(), false)}
          </div>
        </div>
        {/* Hiển thị desktop/tablet - ba cards */}
        <div
          ref={carouselRef}
          className="hidden md:flex gap-5"
        >
          {/* Áp dụng animation cho tất cả các cards khi currentIndex thay đổi */}
          {getDesktopVisibleCards().map((review, index) => (
            <div
              key={review.id}
              className={`w-1/3 ${isAnimating ? 'transition-transform duration-500 ease-in-out transform -translate-x-full' : ''}`}
            >
              {renderReviewCard(review, index === 2)}
            </div>
          ))}
        </div>
      </div>
